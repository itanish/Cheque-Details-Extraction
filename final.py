...
path='./'
tempimg = '_imageToSave.png' 
resizeimg = '_resized_image.png'
micrreferenceimg = 'micr_e13b_reference.png'


@app.route('/postjson/<int:cuid>', methods = ['POST'])
def postJsonHandler(cuid):
    
    success=True

    ifscre = r'[a-zA-Z]{4}0[a-zA-Z0-9]{6}|[a-zA-Z]{4}O[a-zA-Z0-9]{6}' 
    ...
    #print(a)
    '''
    with open("ch1.png", "rb") as imageFile:
    ...
    img = readb64(a,a,cuid)
        
    image = cv2.imread(path+str(cuid)+tempimg)
    ...
    accno = re.findall(bankaccre,str1)
    
    if len(ifsc)==0:
        ifsc[0]='Image not clear'
        success=False
    if len(accno)==0:
        accno[0]='Image not clear'    
        success=False
        
    if len(op[0])>3:
        cheq=op[0][1:-1]
    else:
        cheq='Image not clear'

    data = {
        'success': success,
        'IFSC': ifsc[0],
        'Account Number': accno[0],
        'Cheque Number': cheq,
    }
...
def getMICR(s,cuid):
    ...
    # *black* background
    ref = cv2.imread(micrreferenceimg)
    ...
    # information is)
    image = readb64(str,s,cuid)
    
    #resize
    basewidth = 1200
    image = Image.open(path+str(cuid)+tempimg)
    wpercent = (basewidth / float(image.size[0]))
    hsize = int((float(image.size[1]) * float(wpercent)))
    image = image.resize((basewidth, hsize), Image.ANTIALIAS)
    image.save(path + str(cuid) + resizeimg)    
    
    image = cv2.imread(path + str(cuid) + resizeimg)
    (h, w,) = image.shape[:2]
    delta = int(h - (h * 0.2))
...
def readb64(base64_string,a,cuid):
    
    
    fh = open(path+str(cuid)+tempimg, "wb")
    fh.write(base64.b64decode(a))
    fh.close()
    pimg = cv2.imread(path+str(cuid)+tempimg)
    return pimg    
